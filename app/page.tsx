...
export default function CheckinPage() {
  ...
  // Fade-from-black intro once on mount
  useEffect(() => {
  ...
  return (
    <>
      {/* Intro overlay (fades from black) */}
      {showIntro && <div className="page-intro-overlay" />}

      <div className="min-h-screen bg-background text-foreground animate-fade-in">
        {/* Sticky header with Admin button top-left */}
        <header className="sticky top-0 z-20 border-b border-border bg-card/80 backdrop-blur-sm animate-fade-in animate-delay-100">
          <div className="max-w-6xl mx-auto px-4 md:px-6 py-3 md:py-4 flex items-center justify-between gap-4">
            <Link href="/login">
              <Button
                variant="outline"
                size="sm"
                className="border-border text-primary hover:bg-primary/10 bg-transparent flex items-center gap-1 md:gap-2"
              >
                Admin Dashboard
              </Button>
            </Link>
        ...
        <main className="max-w-6xl mx-auto px-4 md:px-6 py-6 md:py-10 space-y-6 md:space-y-8">
          {/* Logo */}
          <div className="flex justify-center mb-4 animate-scale-in animate-delay-200">
            <Image
              src="/images/home-mark.png"
              alt="Legacy Night Logo"
              width={120}
              height={120}
          ...
          {/* Find Your Seat card */}
          <Card className="bg-card border border-border p-4 md:p-8 shadow-sm animate-slide-up animate-delay-200">
          ...
          {selectedAttendee && (
            <Card className="bg-card border border-border p-4 md:p-6 shadow-sm space-y-4 md:space-y-6 animate-slide-up animate-delay-300">
